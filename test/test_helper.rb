...
require 'app/models/auditor/widget.rb'

def auditor_widget( fixture_name )
  id = @@fixtures['auditor_widget'][ fixture_name.to_s ][ 'id' ]
  Auditor::Widget.find( id )
end
  ...
  fixture.each do |record_name, record|
    record.each do |column, value|
      if ( match = column.match(/(.*)_id/) )
        fixture_reference = "auditor_" + match[1].pluralize
